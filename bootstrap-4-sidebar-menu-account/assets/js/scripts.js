...
jQuery(document).ready(function() {
	...
	/*
	    sidebar-hoivien
	*/
	$('.dismiss-hoivien, .overlay').on('click', function() {
        $('.sidebar-hoivien').removeClass('active');
        $('.overlay').removeClass('active');
    });

    $('.open-menu-hoivien').on('click', function(e) {
    	e.preventDefault();
        $('.sidebar-hoivien').addClass('active');
        $('.overlay').addClass('active');
        // close opened sub-menus
        $('.collapse.show').toggleClass('show');
        $('a[aria-expanded=true]').attr('aria-expanded', 'false');
    });
    /* change sidebar-hoivien style */
	$('a.btn-customized-dark').on('click', function(e) {
		e.preventDefault();
		$('.sidebar-hoivien').removeClass('light');
	});
	$('a.btn-customized-light').on('click', function(e) {
		e.preventDefault();
		$('.sidebar-hoivien').addClass('light');
	});
	/* replace the default browser scrollbar in the sidebar-hoivien, in case the sidebar-hoivien menu has a height that is bigger than the viewport */
	$('.sidebar-hoivien').mCustomScrollbar({
		theme: "minimal-dark"
	});
